...
game, put the world into a state, act, and read back what the engine did — 63 typed
...
**63 tools (+ `exec_enforce` when an allowlist is configured)** across world, player,
...
`dayz_knowledge_find`, `dayz_knowledge_prepare`, `dayz_knowledge_show`, `dayz_knowledge_status`, `dayz_test_close`, `dayz_test_run`, `dayz_test_stop`, `engine_set`, `entities_query`, `infected_drive`, `inventory_attach`, `inventory_give`, `key_press`,
...
- `--client` — what the installer registers. Does not bind; proxies to the daemon
  ...
  It is also the official stdio plan B when the agent host's `mcp.list_tools`
  raises `McpStartupError` (vsock) and never recovers — spawn this process
  yourself, speak MCP JSON-RPC on stdin/stdout, and close it at session end.
  See [tools/README-mcp.md#host-vsock-failure-official-stdio-plan-b](tools/README-mcp.md#host-vsock-failure-official-stdio-plan-b).
