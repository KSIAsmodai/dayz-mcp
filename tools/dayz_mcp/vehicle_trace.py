...
DUMP_FILENAME_PREFIX = "dayz_mcp_trace_"
...
def load_dump_jsonl(path: Path) -> dict[str, object]:
    try:
    ...
    if not isinstance(header, dict):
        raise ValueError("dump_invalid")
    if header.get("schema") != TRACE_SCHEMA or header.get("mode") != "dump":
        raise ValueError("dump_invalid")
    trace_id = header.get("trace_id")
    rows = header.get("rows")
    count = header.get("count")
    if not _is_canonical_dump_path(header.get("path"), trace_id):
        raise ValueError("dump_invalid")
    samples: list[object] = []
    for line in lines[1:]:
        try:
    ...
    if not _is_int(rows) or not _is_int(count) or rows != count or rows != len(samples):
        raise ValueError("dump_count_mismatch")
    if rows < 0 or rows > 8192:
        raise ValueError("dump_invalid")
    trace = dict(header)
    trace["mode"] = "read"
...
def classify_14de_throttle_sample(sample: dict[str, object]) -> str:
    ...
    Wire JSON 1/0 uses `_normalize_bridge_boolean`. Does not mutate the sample.
    """
    try:
        forced = _normalize_bridge_boolean(sample["forced"])
    except ValueError:
        forced = False
    if forced:
        return "forced"
    try:
        control_active = _normalize_bridge_boolean(sample["control_active"])
    except ValueError:
        control_active = False
    if not control_active:
        return "no_control"
    try:
        ready = _normalize_bridge_boolean(sample["engine_ready"])
    except ValueError:
        ready = False
    try:
        throttle_set = _normalize_bridge_boolean(sample["throttle_set"])
    except ValueError:
        throttle_set = False
