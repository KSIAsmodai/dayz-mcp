...
original, ver `dayz-mcp-architecture.md` §4; hoy son 62 tools registradas, ver
...
| H4 | Cola FIFO estricta salvo ventana de gracia post-TTL de 90 s en la que solo la identidad titular a t=TTL puede acquire/wait-claim si a t=TTL tenía un RUNNING propio (snapshot antes de release_owner); el token sigue inválido a 120 s; extraños solo promocionan con session_wait vivo después de la gracia; como máximo 1 reacquire preferente consecutivo cuando la cola no está vacía; `acquire` idempotente | A→B→C, ticket duplicado no duplica posición, reloj inyectable 119/120/121 s; release/expiry sin `session_wait` vivo no conceden leases; gracia 90 s con probe de RUNNING; B no 200 durante la ventana | ✓ offline |
...
| H13 | Diagnóstico seguro de lifecycle y runs compartidos | Preflight detecta `Steam ActiveProcess` obsoleto antes de lanzar, devuelve PID registrado/vivos acotados más remedio seguro y readiness no equivale a PID vivo; reattach `mode=client` conserva el run y los errores de la matriz `mode × run_id` nombran la restricción concreta; ocupación y `active_run_exists` informan edad/actividad, y el audit existente explica generation de runs activos o retirados sin preemptar. Gates pos/neg/inconclusos prueban cada cláusula y todo fallo de identidad, path, proceso o fuente falla cerrado. Cuando la caja está ocupada por un run gestionado en `RUNNING`/`RUNNING_IDLE` que el caller no posee, el error MCP de `dayz_test_run` es `takeover_required` con `occupied_by_run_id`, no `active_run_exists`. `active_run_exists` se conserva para el resto de ocupaciones (transitorias, sin `state`, puerto foreign) y para el wire sellado de lifecycle `start_run`. | ❓ |
| H14 | Lectura H3 y `dayz_test_stop` propio tras fallo de `policy.revalidate()` | Un cliente en `client_policy_untrusted_open_new_session` conserva dos verbos lícitos. **(a)** `server_reload`: camino primario de reacreditación; el supervisor lo intercepta en `tools/call` y no pasa por `ControlClient.revalidate`. **(b)** Excepción enumerada: las tools de lectura de H3 y `dayz_test_stop` de un run que `_caller_owns_run` acepta (`item.owner_session` ∈ `{caller_session, caller_session[:12]}`) no se rechazan por el fallo de `policy.revalidate()` ni en `ControlClient._request_once` ni en `daemon_credential._revalidate_authority`. `_assert_authority_unchanged` sigue fail-closed: policy que ya no es `AccreditedDaemonPolicy`, rebind, PID reuse o tupla de autoridad distinta → `client_policy_untrusted_open_new_session` con `http_bytes_sent=0`. La petición eximida viaja con la credencial ya emitida hacia el daemon actual. `owner_session` identifica al **cliente MCP**, no a la sesión Claude (`9195` punto 4). El stop sigue siendo forced kill de **ese** único run (H11). `session_acquire_wait` **no** está eximido. Taskkill manual sigue prohibido. Todo otro verbo del daemon sigue fail-closed con `http_bytes_sent=0`. | ✓ offline |
...
- **2026-09-12 (H14 — lectura H3 y stop propio bajo `policy.revalidate()` fallido):** Grill W4 Decisión 4 (`050e`). Nuevo criterio **H14** y enmienda a **H10**: un cliente `client_policy_untrusted_open_new_session` conserva `server_reload` y, como red de seguridad, las lecturas H3 más `dayz_test_stop` de un run que `_caller_owns_run` acepta. El fail-closed de `policy.revalidate()` se omite en `ControlClient._request_once` y `daemon_credential._revalidate_authority` para esos verbos; `_assert_authority_unchanged` sigue fail-closed (`http_bytes_sent=0`). `session_acquire_wait` no está eximido. `owner_session` es el cliente MCP, no la sesión Claude (`9195` punto 4). `050e` se aparca a esta cita; no se marca resuelta hasta verificación viva. No reabre PARO `3fc1`/`1025`.
